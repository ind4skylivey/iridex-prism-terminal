...
echo "==> Release pipeline overview for ${version}"
cat <<EOF
1. Run fmt/clippy/tests plus prompt/dotfile/widget benches and compare against perf baselines.
2. Bump Cargo.toml/Cargo.lock metadata and regenerate metadata for ${version}.
3. Stamp CHANGELOG.md with the ${version} release date (${release_date}).
4. Build release binaries, tar them into dist/${version}, and optionally sign with gpg.
5. Emit the manual git/tag/publish checklist once artifacts are ready.
EOF
