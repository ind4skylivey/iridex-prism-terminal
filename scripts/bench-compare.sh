...
Compares the latest Criterion benchmark artifacts under target/criterion
against a baseline JSON (default benchmarks/perf-baseline.json). Exits non-zero
if any named metric regresses (mean or median) by more than the configured
threshold (default 5%).
EOF
}
