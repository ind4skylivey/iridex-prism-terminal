...
fn handle_dotfiles(args: DotfileArgs) -> PrismResult<()> {
    match args.command {
        ...
        DotfileCommands::RestoreAll { destination } => restore_all_dotfiles(destination),
        DotfileCommands::Exclude { name } => exclude_dotfile(&name),
        DotfileCommands::Include { name } => include_dotfile(&name),
        DotfileCommands::Exclusions => list_exclusions(),
    }
}
...
fn restore_dotfile(name: &str, destination: Option<PathBuf>) -> PrismResult<()> {
    let resolved = resolve_dotfile_name(name)?;
    let target = match destination {
        Some(path) => finalize_destination(path, &resolved),
        None => dotfiles::default_restore_destination(&resolved)?,
    };
    let outcome = dotfiles::restore_to(&resolved, &target)?;
...
fn restore_all_dotfiles(destination: Option<PathBuf>) -> PrismResult<()> {
    let files = dotfiles::tracked_files()?;
    if files.is_empty() {
        println!("No tracked dotfiles to restore.");
        return Ok(());
    }
    let base = match destination {
        Some(path) => path,
        None => dirs::home_dir().ok_or_else(|| PrismError::new("missing home directory"))?,
    };
    let base_dir = if base.exists() && base.is_file() {
        base.parent()
            .map(PathBuf::from)
            .ok_or_else(|| PrismError::new("destination must be a directory"))?
    } else {
        base
    };
    ...
    for path in files {
        if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
            ...
            let outcome = dotfiles::restore_to(name, &target)?;
            if let Some(backup) = outcome.overwritten_backup {
                println!(
                    "Backed up existing {} to {}",
                    target.display(),
...
fn exclude_dotfile(name: &str) -> PrismResult<()> {
    let entry = resolve_dotfile_name(name)?;
    dotfiles::add_exclusion(&entry)?;
    println!(
        "Excluded '{entry}' from sync backups (stored in {}).",
        dotfiles::EXCLUSIONS_FILE
    );
    Ok(())
}

fn include_dotfile(name: &str) -> PrismResult<()> {
    let entry = resolve_dotfile_name(name)?;
    dotfiles::remove_exclusion(&entry)?;
    println!("Including '{entry}' in dotfile sync again.");
    Ok(())
}

fn list_exclusions() -> PrismResult<()> {
    let entries = dotfiles::load_exclusions()?;
    if entries.is_empty() {
        println!("No dotfiles excluded (sync will track everything).");
    } else {
        println!("Excluded dotfiles (from {}):", dotfiles::EXCLUSIONS_FILE);
        for name in entries {
            println!("- {name}");
        }
    }
...
fn resolve_dotfile_name(input: &str) -> PrismResult<String> {
    let trimmed = input.trim();
    if trimmed.is_empty() {
        return Err(PrismError::new("dotfile name cannot be empty"));
    }
...
mod tests {
    ...
    #[test]
    fn resolve_dotfile_name_from_path() {
        let name = resolve_dotfile_name("/tmp/prism/.zshrc").expect("resolved");
        assert_eq!(name, ".zshrc");
    }

    #[test]
    fn resolve_dotfile_name_rejects_empty() {
        assert!(resolve_dotfile_name("   ").is_err());
    }
}
